...
if($datosF->ActualizarServiciosEmpresariales($tiposervicio,$estadoservicio,$formato_contrato,$num_contrato,$fecha_inicio,$fecha_fin,$asesorcomercial,$descripcion_contrato,$datosF->cod_ser)){
                if($datosF->ActualizarDetallesEmpresariales($tipoconex,$velocidadmax,$velocidadmin,$nodo,$antena,$datosF->cod_det)){            
                    ...
                    for($i=1;$i <= $comparadoripB;$i++){
                        if(empty($_POST['direccionIp'.$i])){
                         ...
                         if($datosF->ActualizarIpBackEmpresariales($direccionIp1,$descripcionIp1,$cod_bak)){
                            $var=1;
                         }else{
                            $var=0;
                         }
                    ...
                    if($comparadoripBack != 1){
                        for($i=$comparadoripB+1;$i <= $comparadoripBack;$i++){
                            if(empty($_POST['direccionIp'.$i])){
                             ...
                             $direccionIp1=$_POST['rangoipB'.$i].$cod;
                             $descripcionIp1=$_POST['descripcionIp'.$i];
                             if($datosF->RegistrarIpBackboneEmp($direccionIp1,$descripcionIp1,$datosF->cod_det)){
                                $var=1;
                             }else{
                                $var=0;
                             }
                    ...
                    for($i=1;$i <= $comparadoripC;$i++){
                        if(empty($_POST['direccionIpc'.$i])){
                         ...
                         if($datosF->ActualizarIpClientesEmpresariales($direccionIpc1,$descripcionIpc1,$cod_cli)){
                            $var=1;
                         }else{
                            $var=0;
                         }
                    ...
                    if($comparadoripCli != 1){
                        for($i=$comparadoripC+1;$i <= $comparadoripCli;$i++){
                            ...
                            $cod=$_POST['direccionIpc'.$i];
                             ...
                             $direccionIpc1=$_POST['rangoipC'.$i].$cod;
                             $descripcionIpc1=$_POST['descripcionIpc'.$i];
                             if($datosF->RegistrarIpClienteEmp($direccionIpc1,$descripcionIpc1,$datosF->cod_det)){
                                $var=1;
                             }else{
                                $var=0;
                             }
                    ...
                    $comparadoripE=$_POST['Equipo'];
                      for($i=1;$i <= $comparadoripE;$i++){
                         ...
                         if($datosF->ActualizarIpEquiposEmpresariales($direccionIpe1,$direccionmac,$descripcionIpe1,$cod_equ)){
                            $var=1;
                         }else{
                            $var=0;
                         }
                    ...
                    $comparadoripEqui=$_POST['comparadoripEq'];
                     if($comparadoripEqui != 1){
                          for($i=$comparadoripE+1;$i <= $comparadoripEqui;$i++){
                             ...
                             $direccionmac=$_POST['direccionmac'.$i];
                             $descripcionIpe1=$_POST['descripcionserial'.$i];
                             if($datosF->RegistrarIpEquipoEmp($direccionIpe1,$direccionmac,$descripcionIpe1,$datosF->cod_det)){
                                $var=1;
                             }else{
                                $var=0;
                             }
                ...
                +<button type="button" onclick=' ListarServiciosActualizarDatos("controlador/listarServicioActualizarEmpresas")' class="btn btn-primary ac" data-dismiss="modal"><span class="glyphicon glyphicon-chevron-left"></span>  Verificar</button>
                    <?php
                    }else{
